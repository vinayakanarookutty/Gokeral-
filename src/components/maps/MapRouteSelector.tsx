...
const MapRouteSelector: React.FC = () => {
  ...
  // 🔊 Handle voice-triggered route setting
  useEffect(() => {
    const handler = (e: any) => {
      const { origin, destination } = e.detail;
      console.log("VOICE ROUTE RECEIVED:", { origin, destination });
    ...
    window.addEventListener("voice-route-set", handler);
    return () => window.removeEventListener("voice-route-set", handler);
  }, []);
            ...
            {availableRoutes.map((routeInfo, index) => (
              <div
                ...
                className={`p-3 border rounded cursor-pointer ${
                  selectedRouteIndex === index ? "bg-blue-100 border-blue-300" : "bg-white border-gray-200"
                }`}
